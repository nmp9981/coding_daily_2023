...
int vedioCount[maxi];//각 시각별 동영상 총 개수
int addTime;//광고 재생시간
int playTime;//총 재생시간
string answer = "";//정답
...
void CountVideo(){
    ...
    for(int i=1;i<=playTime;i++) vedioCount[i] += vedioCount[i];//시각별 총 동영상 개수
}
//광고 넣기
void AD_Insert(){
    for(int i=1;i<=playTime;i++) vedioCount[i] += vedioCount[i];//누적 시청자 수
    int AD_start = 0;int maxCount = vedioCount[addTime-1];
    for(int i=1;i<=playTime-addTime+1;i++){
        int people = vedioCount[i+addTime-1]-vedioCount[i-1];
